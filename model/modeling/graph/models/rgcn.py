...
from ..layers import GaussianTransformation
from ..layers import GaussianPropagation
from ..utils import Prediction, ModelConfiguration
...
class RGCN(Model):
    ...
    def forward_impl(self, data: Data) -> Prediction:
        ...
        for _ in range(self.num_sample):
            eps = torch.zeros_like(mu_1p).normal_()
            sample_z.append(mu_1p + eps * torch.sqrt(var_1p + self.var_eps))
        sample_z = torch.cat(sample_z, dim=1)
        # sample_z = torch.stack(sample_z).mean(dim=0)
        ...
        for _ in range(self.num_sample):
            ...
            z.append(mu_2p + eps * torch.sqrt(var_2p + self.var_eps))
        z = torch.stack(z).mean(dim=0)
        ...
        pred = Prediction(
            ...
            # sample sampling
            softs=sample_z.mean(dim=1),
            var=sample_z.var(dim=1),

            # confidence of prediction
            prediction_confidence_aleatoric=max_soft,
            prediction_confidence_epistemic=1.0 / (var_2p[torch.arange(hard.size(0)), hard] + self.var_eps), # 1.0 / 
            prediction_confidence_structure=(var_2p[torch.arange(hard.size(0)), hard] + self.var_eps),
            # prediction_confidence_structure=None,

            # confidence of sample
            sample_confidence_aleatoric=max_soft, # sample_z.mean(dim=1)
            ...
            sample_confidence_structure=(var_1.sum(-1) + self.var_eps), # sample_z.var(dim=1)
    ...
    def KL(self, prediction: Prediction) -> Dict[str, Tensor]:
        ...
        kl = 0.5 * (var + mu ** 2 - 1 - (var + 3.0e-8).log()).mean(dim=-1).sum()
