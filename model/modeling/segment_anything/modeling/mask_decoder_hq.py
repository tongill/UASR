...
from transformers.models.blip_2.modeling_blip_2 import Blip2Config, Blip2QFormerAttention
...
class MaskDecoderHQ(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.cross_attention = Blip2QFormerAttention(config, is_cross_attention=True)
    ...
    def forward(
        ...
        **kwargs
        ...
        """
        try:
            input_cat_embeddings = kwargs.pop("input_cat_embeddings")
        except:
            pass
    ...
    def predict_masks(
        ...
        dense_attention_mask = torch.ones(dense_prompt_embeddings.size()[:-1], dtype=torch.long, device=dense_prompt_embeddings.device)
        cross_attention_outputs = self.cross_attention(
            hidden_states=src.reshape(b, c, -1).permute(0, 2, 1),
            encoder_hidden_states=dense_prompt_embeddings,
            encoder_attention_mask=dense_attention_mask,
            output_attentions=False,
        )
        query_attention_output = cross_attention_outputs[0]
        src = query_attention_output.permute(0, 2, 1).reshape(src.shape)
